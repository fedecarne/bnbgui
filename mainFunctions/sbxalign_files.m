function r = sbxalign_files(A,idx)
 
if(length(idx)==1)
 
 r.m{1} = A(:,:,idx); % mean
 r.m{2} = zeros(size(A(:,:,idx))); % 2nd moment
 r.m{3} = zeros(size(A(:,:,idx))); % 3rd moment
 r.m{4} = zeros(size(A(:,:,idx))); % 4th moment
 ...
 r0 = sbxalign_files(A,idx0); % align each group
 r1 = sbxalign_files(A,idx1);
 ...
 r.m{3} = r0.m{3} + r1.m{3} + delta.^3 * na * nb * (na-nb)/nx^2 + 3 * delta / nx .* (na * r1.m{2} - nb * r0.m{2});
 r.m{4} = r0.m{4} + r1.m{4} + delta.^4 * na * nb * (na^2 - na * nb + nb^2) / nx^3 + 6 * delta.^2 .* (na^2 * r1.m{2} + nb^2 * r1.m{2}) / nx^2 + 4 * delta .* (na * r1.m{3} - nb * r0.m{3}) / nx;
