...
function setcallbacks(src,evnt)
    ...
    switch evnt.Key
        ...
        case 'l'
            label_roi_Callback(hObject,0, handles);
        case 'escape'
        case 'v'
            see_roi_Callback(hObject,0, handles);
        case '1'
...
handles.label_txt.String = handles.labelList(handles.roi_label(handles.roiList.Value));
guidata(hObject, handles);
...
%         handles.rois_data(roi_num).neuropil = find(neuropil_mask);
    ...
    for i=1:size(roi_id,1)
        ...
        rois_data(i).label_index = handles.roi_label(i);
        rois_data(i).label = handles.labelList(handles.roi_label(i));
    end
                
    rois=handles.rois.CData;
    
    PathName = uigetdir(pwd,'Save file name');
    
%    [FileName,PathName] = uiputfile('rois_data.mat','Save file name');
    save([PathName '/rois_data'],'rois_data')
    
%    [FileName,PathName] = uiputfile('rois.mat','Save file name');
    save([PathName '/rois'],'rois')

%    [FileName,PathName] = uiputfile('rois_fig.mat','Save roi reference image');
    aux = handles.baseimage;
    save([PathName '/roi_fig'],'aux')
...
% --- Executes on button press in show_rois_tgl.
function show_rois_tgl_Callback(hObject, eventdata, handles)
% hObject    handle to show_rois_tgl (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if handles.see_roi_current;
    handles.show_rois_tgl.Value = get(handles.show_rois_tgl,'Min');
    handles.roi_image.Visible='Off'; 
    handles.see_roi_current=0;
else
    handles.show_rois_tgl.Value = get(handles.show_rois_tgl,'Max');
    handles.roi_image.Visible='On'; 
	handles.see_roi_current=1;
end
guidata(hObject, handles);

% Hint: get(hObject,'Value') returns toggle state of show_rois_tgl


% --- Executes on button press in erase_btn.
function erase_btn_Callback(hObject, eventdata, handles)    
% hObject    handle to erase_btn (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
axes(handles.lowAxes);
[x, y] = getline(handles.lowAxes,'closed');
BW = poly2mask(x, y, handles.lowAxes.YLim(2), handles.lowAxes.XLim(2));
handles.low_image.CData(BW,1)=0;
